...
export function ExtendedIndicatorPanel({ kpi, filters, onFilters }: Props) {
  ...
  return (
    <section className="bi-page extended-bi" data-panel={`kpi${String(kpi).padStart(2, "0")}`}>
      ...
      {unavailable ? <section className="panel unavailable"><span className="eyebrow">HARD GATE</span><h2>Indicador não publicado como número</h2><p>{data?.reason}</p>{data?.rule ? <div className="management-note"><strong>Regra</strong><p>{data.rule}</p></div> : null}{data?.context ? <div className="management-note"><strong>Contexto disponível</strong><p>{Object.entries(data.context).map(([key, value]) => `${key}: ${String(value)}`).join(" · ")}</p></div> : null}</section> : null}
      ...
      {data && kpi === 4 && metrics ? <>
        ...
        <section className="bi-layout-main"><article className="panel"><div className="panel-heading"><div><span className="eyebrow">EVOLUÇÃO</span><h2>Mediana por mês de saída</h2></div></div><InteractiveBars items={rowsToVisuals(data.monthly, "median_days")} onSelect={(item) => toggle("month", String(Number(item.key.slice(5, 7))))} tone="teal" initialLimit={12} /></article><article className="panel"><div className="panel-heading"><div><span className="eyebrow">DISTRIBUIÇÃO</span><h2>Faixas de tramitação</h2></div></div><InteractiveBars items={rowsToVisuals(data.bands)} onSelect={() => undefined} tone="orange" initialLimit={10} /></article></section>
        <article className="panel"><div className="panel-heading"><div><span className="eyebrow">CATEGORIAS</span><h2>Mediana por categoria</h2></div></div><InteractiveBars items={rowsToVisuals(data.categories, "median_days")} selected={filters.category} onSelect={(item) => toggle("category", item.key)} tone="teal" initialLimit={12} /></article><DetailTable records={data.records} />
      </> : null}
