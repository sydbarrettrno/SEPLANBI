...
test("filtros globais, pesquisa e limpeza afetam o mesmo universo analítico", async ({ page }) => {
  ...
  await waitForCount(page, 2159);
  await openFilters(page);
  const filtersDialog = page.getByRole("dialog", { name: "Filtros do painel" });
  await expect(page.getByLabel("De", { exact: true })).toHaveCount(0);
  await expect(page.getByLabel("Até", { exact: true })).toHaveCount(0);
  await expect(filtersDialog.getByRole("combobox", { name: "Ano de abertura", exact: true })).toBeVisible();
  await expect(filtersDialog.getByRole("combobox", { name: "Mês de abertura", exact: true })).toBeVisible();
  await filtersDialog.getByRole("combobox", { name: "Status", exact: true }).selectOption("Em Análise");
  await filtersDialog.getByRole("combobox", { name: "Setor de tramitação", exact: true }).selectOption({ index: 1 });
  await filtersDialog.getByRole("combobox", { name: "Responsabilidade", exact: true }).selectOption("Interno");
